...
function MainLayout({ children }) {
  ...
  useEffect(() => {
    ...
    } else if (isUserAdmin || isSuperAdmin) {
      ...
      <div className={`${mobileSidebarOpen ? 'block' : 'hidden'} fixed top-0 left-0 h-screen z-50 ${isUserRSORepresentative ? `bg-white` : `bg-primary`} w-[300px]  p-6 transition-ease-in-out duration-300 xl:hidden`}>
        ...
        <div className="mt-6 flex flex-col gap-2 h-full  ">
          ...
          <SidebarButton
            ...
            text={isUserRSORepresentative ? "Documents" : isUserAdmin ? "Dashboard" : "User Management"}
            active={isUserRSORepresentative ? (location.pathname === "/document") : isUserAdmin ? (location.pathname.startsWith("/dashboard")) : isSuperAdmin ? (location.pathname === "/user-management") : false}
            onClick={() => navigate(isUserRSORepresentative ? "/document" : isUserAdmin ? "/dashboard" : isSuperAdmin ? "/user-management" : "/userProfileError")}
          />
          ...
          <SidebarButton
            ...
            text={isUserRSORepresentative ? "RSO Account" : isUserAdmin ? "Admin Account" : isSuperAdmin ? "Super Admin Account" : "User Account"}
